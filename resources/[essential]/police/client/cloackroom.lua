...
buttons[#buttons+1] = {name = "Clock in as Female Officer", func = "clockIn_UniformedF"}
buttons[#buttons+1] = {name = "Clock in as State Trooper", func = "clockIn_stateT"}
buttons[#buttons+1] = {name = "Clock in as Detective", func = "clockIn_detective"}
buttons[#buttons+1] = {name = "Clock In as Undercover", func = "clockIn_Undercover"}
...
function clockIn_Uniformed()
	-- Citizen.Trace("BOL3")
	ServiceOn()
	giveUniforme("s_m_y_cop_01")
	drawNotification(txt[config.lang]["now_in_service_notification"])
	drawNotification(txt[config.lang]["help_open_menu_notification"])
end

function clockIn_UniformedF()
	-- Citizen.Trace("BOL3")
	ServiceOn()
	giveUniforme("s_f_y_cop_01")
	drawNotification(txt[config.lang]["now_in_service_notification"])
	drawNotification(txt[config.lang]["help_open_menu_notification"])
end

function clockIn_stateT()
	-- Citizen.Trace("BOL3")
	ServiceOn()
	giveUniforme("s_m_y_hwaycop_01")
	drawNotification(txt[config.lang]["now_in_service_notification"])
	drawNotification(txt[config.lang]["help_open_menu_notification"])
end


function clockIn_detective()
	-- Citizen.Trace("BOL3")
	ServiceOn()
	giveUniforme("s_m_m_ciasec_01")
	drawNotification(txt[config.lang]["now_in_service_notification"])
	drawNotification(txt[config.lang]["help_open_menu_notification"])
end
