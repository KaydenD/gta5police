...
function isNearBairCarGarage()
	local distance = 10000
	local pos = {}
	for i = 1, #baitStation do
		local coords = GetEntityCoords(GetPlayerPed(-1), 0)
		local currentDistance = Vdist(baitStation[i].x, baitStation[i].y, baitStation[i].z, coords.x, coords.y, coords.z)
		if(currentDistance < distance) then
			distance = currentDistance
			pos = baitStation[i]
		end
	...
	if anyMenuOpen.menuName == "garage1" and anyMenuOpen.isActive and distance > 5 then
		CloseMenu()
	end
	if(distance < 30) then
