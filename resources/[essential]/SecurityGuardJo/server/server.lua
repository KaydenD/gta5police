...
function addCop(identifier)
	if(db.driver == "mysql-async") then
		MySQL.Async.fetchAll("SELECT * FROM seq WHERE identifier = '"..identifier.."'", { ['@identifier'] = identifier}, function (result)
			if(result[1] == nil) then
				MySQL.Async.execute("INSERT INTO seq (`identifier`) VALUES ('"..identifier.."')", { ['@identifier'] = identifier})
			end
		end)
...
function remCop(identifier)
	if(db.driver == "mysql-async") then
		MySQL.Async.execute("DELETE FROM seq WHERE identifier = '"..identifier.."'", { ['@identifier'] = identifier})
	elseif(db.driver == "couchdb") then
		couchFunctions.getDocumentByRow("police", "identifier", identifier, function(document)
			if(document ~= false) then
...
if(config.useCopWhitelist) then
	...
	TriggerEvent('es:addGroupCommand', 'seqadd', "admin", function(source, args, user)
		 if(not args[2]) then
	...
	TriggerEvent('es:addGroupCommand', 'seqrem', "admin", function(source, args, user) 
		 if(not args[2]) then
			print("nein")
