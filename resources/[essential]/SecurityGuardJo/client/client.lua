...
local baitStation = {
	{x=447.36532592773, y=-997.51257324219, z=24.75659942627}
}
...
AddEventHandler('jail:teleportPlayer', function(amount, cop, override)
	if(isHandCuffed == true or override == true) then
		SetEntityCoords(GetPlayerPed(-1), 1652.4632568359, 2614.9792480469, 45.564884185791)
		...
		TriggerServerEvent("police:removeWeapons1")
		TriggerEvent('chatMessage', '^4[JAIL]', {0,0,0}, "Your Weapons got removed for being Arrest!")
		if(override == false) then
		TriggerEvent('police:getArrested')
		TriggerServerEvent('InsertJailDBTime', tonumber(amount))
		end
		SetEntityCanBeDamaged(GetPlayerPed(-1), false)
		TriggerServerEvent('arrestNotify', amount)
		Citizen.CreateThread(function()
			while (amount > 0) do
				Citizen.Wait(1000)
				amount = amount - 1
				if (oldamount - 30 >= amount) then
					oldamount = amount
					TriggerServerEvent('arrestNotify', amount)
					TriggerServerEvent('UpdateJailDBTime', tonumber(amount))
				...
				if ((Vdist(1652.4632568359, 2614.9792480469, 45.564884185791, plyCoords["x"], plyCoords["y"], plyCoords["z"])) > 75) then 
					SetEntityCoords(ply, 1652.4632568359, 2614.9792480469, 45.564884185791)
				end
			end
			SetEntityCoords(GetPlayerPed(-1), 162.22509765625, -978.78332519531, 30.09193611145)
			SetEntityCanBeDamaged(GetPlayerPed(-1), true)
			TriggerServerEvent("DeleteJailDB")
		end)
	else
		--TriggerEvent('chatMessage', source,'^4[JAIL]', {0,0,0}, "Player need to be cuffed!")
		TriggerServerEvent("NotifyCop", cop)
	end
end)

RegisterNetEvent('ragdoll')
AddEventHandler('ragdoll', function()
	if(IsPedInAnyVehicle(GetPlayerPed(-1), true) == false and handCuffed == false) then
...
function GetPlayerDistance(targetId)
	...
	local target = GetPlayerPed(GetPlayerFromServerId(tonumber(targetId)))
	if(target ~= ply) then
		local targetCoords = GetEntityCoords(target, 0)
		return Vdist(targetCoords["x"], targetCoords["y"], targetCoords["z"], plyCoords["x"], plyCoords["y"], plyCoords["z"])
...
function isNearSeq()
	local distance = 10000
	local pos = {}
	for i = 1, #seqStation do
		local coords = GetEntityCoords(GetPlayerPed(-1), 0)
		local currentDistance = Vdist(seqStation[i].x, seqStation[i].y, seqStation[i].z, coords.x, coords.y, coords.z)
		if(currentDistance < distance) then
			distance = currentDistance
			pos = seqStation[i]
		end
	...
	if anyMenuOpen.menuName == "garage2" and anyMenuOpen.isActive and distance > 5 then
		CloseMenu()
	end
	if(distance < 30) then
		DrawMarker(1, pos.x, pos.y, pos.z, 0, 0, 0, 0, 0, 0, 2.0, 2.0, 1.0, 0, 155, 255, 200, 0, 0, 2, 0, 0, 0, 0)
	end
	if(distance < 2) then
		return true
	end
...
function isCopCar(plate)
    if(string.find(plate, "COP")) then
        return true
    else
        return false
    end
...
Citizen.CreateThread(function()
	...
	Citizen.Wait(0)
		if(isInService) then		
			if (IsPedInAnyPoliceVehicle(GetPlayerPed(-1)) or (GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1))) == GetHashKey("riot"))) then --Insert VehicleModelName here.
				if (GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1)),-1) == GetPlayerPed(-1)) then 
					SetVehicleEnginePowerMultiplier(GetVehiclePedIsIn(GetPlayerPed(-1), true), 10.0)
					SetVehicleEngineTorqueMultiplier(GetVehiclePedIsIn(GetPlayerPed(-1), true), 1.6)
				end
			end
		end
	end
    ...
    while true do
		...
		if(anyMenuOpen.isActive) then
			...
			elseif (IsControlJustPressed(1,177)) then
				if(anyMenuOpen.menuName == "policemenu" or anyMenuOpen.menuName == "armory" or anyMenuOpen.menuName == "cloackroom" or anyMenuOpen.menuName == "garage2") then
					CloseMenu()
        ...
        if(isCop) then
			...
			if(isInService) then
				...
				if(isNearBairCarGarage()) then
					DisplayHelpText("Press E to get a bait car",0,1,0.5,0.8,0.6,255,255,255,255)
				...
				if(isNearSeq()) then
					DisplayHelpText("Press E to get a car",0,1,0.5,0.8,0.6,255,255,255,255)
				...
				if isNearHelicopterStation() then
					...
					if IsControlJustPressed(1,51)  then
						if(policeHeli ~= nil) then
							...
							local heli = GetHashKey("supervolito")
							local ply = GetPlayerPed(-1)
							local plyCoords = GetEntityCoords(ply, 0)
