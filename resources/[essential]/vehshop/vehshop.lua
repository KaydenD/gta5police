...
{entering = {-33.803,-1102.322,25.422}, inside = {-46.56327,-1097.382,25.99875, 120.1953}, outside = {-31.849,-1090.648,25.998,322.345}, shopNum = 1},
{entering = {-52.525810241699,-1688.0159912109,28.49174118042}, inside = {-52.525810241699,-1688.0159912109,28.49174118042}, outside = {-52.525810241699,-1688.0159912109,28.49174118042}, shopNum = 2},
}
...
function getHash(hash)
	for i,v in pairs(vehList) do
		if(GetHashKey(i) == hash) then
			--Citizen.Trace(v.torqeBoost)
			return tonumber(v.torqeBoost)
		end
	end
	--Citizen.Trace("1.0")
	return 1.0
...
Citizen.CreateThread(function()
	...
	Citizen.Wait(0)
		if(loaded) then		
			if not (IsPedInAnyPoliceVehicle(GetPlayerPed(-1))) then --Insert VehicleModelName here.
				if (GetPedInVehicleSeat(GetVehiclePedIsIn(GetPlayerPed(-1)),-1) == GetPlayerPed(-1)) then 	
					--Citizen.Trace("TEST")
					SetVehicleEngineTorqueMultiplier(GetVehiclePedIsIn(GetPlayerPed(-1), true), getHash(GetEntityModel(GetVehiclePedIsIn(GetPlayerPed(-1)))))
				end
			end
		end
	end
		...
		Citizen.CreateThread(function()
			while #vehshop_blips > 0 do
				...
				for i,b in ipairs(vehshop_blips) do
					if IsPlayerWantedLevelGreater(GetPlayerIndex(),0) == false and vehshop[b.pos.shopNum].opened == false and IsPedInAnyVehicle(LocalPed(), true) == false and  GetDistanceBetweenCoords(b.pos.entering[1],b.pos.entering[2],b.pos.entering[3],GetEntityCoords(LocalPed())) < 5 then
						DrawMarker(1,b.pos.entering[1],b.pos.entering[2],b.pos.entering[3],0,0,0,0,0,0,2.001,2.0001,0.5001,0,155,255,200,0,0,0,0)
						drawTxt('Press ~g~ENTER~s~ to buy ~b~vehicle',0,1,0.5,0.8,0.6,255,255,255,255)
						currentlocation = b
						inrange = true
						shopNum = b.pos.shopNum
					elseif (vehshop[b.pos.shopNum].opened) then
						shopNum = b.pos.shopNum
						currentlocation = b
					end
				end
...
function OpenCreator()		
	...
	vehshop[closeShop].currentmenu = vehshop[currentlocation.pos.shopNum].mainMenu
	vehshop[closeShop].opened = true
	vehshop[closeShop].selectedbutton = 0
	--[[Citizen.CreateThread(function()
...
function CloseCreator()
	Citizen.CreateThread(function()
		...
		if not boughtcar then
			...
			--local instance = GetClosestVehicle(pos[1], pos[2], pos[3], 2.000, 0, 70)
...
local neoncolor = table.pack(GetVehicleNeonLightsColour(personalvehicle))
local mods = table.pack(GetVehicleMod(personalvehicle))
local smokecolor = table.pack(GetVehicleTyreSmokeColor(personalvehicle))
local plate = GetVehicleNumberPlateText(personalvehicle)
local plateindex = GetVehicleNumberPlateTextIndex(personalvehicle)
...
local windowtint = GetVehicleWindowTint(personalvehicle)
local wheeltype = GetVehicleWheelType(personalvehicle)
local smokecolor1 = smokecolor[1]
local smokecolor2 = smokecolor[2]
local smokecolor3 = smokecolor[3]
local mods0 = GetVehicleMod(personalvehicle, 0)
local mods1 = GetVehicleMod(personalvehicle, 1)
local mods2 = GetVehicleMod(personalvehicle, 2)
local mods3 = GetVehicleMod(personalvehicle, 3)
local mods4 = GetVehicleMod(personalvehicle, 4)
local mods5 = GetVehicleMod(personalvehicle, 5)
local mods6 = GetVehicleMod(personalvehicle, 6)
local mods7 = GetVehicleMod(personalvehicle, 7)
local mods8 = GetVehicleMod(personalvehicle, 8)
local mods9 = GetVehicleMod(personalvehicle, 9)
local mods10 = GetVehicleMod(personalvehicle, 10)
local mods11 = GetVehicleMod(personalvehicle, 11)
local mods12 = GetVehicleMod(personalvehicle, 12)
local mods13 = GetVehicleMod(personalvehicle, 13)
local mods14 = GetVehicleMod(personalvehicle, 14)
local mods15 = GetVehicleMod(personalvehicle, 15)
local mods16 = GetVehicleMod(personalvehicle, 16)
local mods23 = GetVehicleMod(personalvehicle, 23)
local mods24 = GetVehicleMod(personalvehicle, 24)
if IsToggleModOn(personalvehicle,18) then
	turbo = "on"
else
	turbo = "off"
end
if IsToggleModOn(personalvehicle,20) then
	tiresmoke = "on"
else
	tiresmoke = "off"
end
if IsToggleModOn(personalvehicle,22) then
	xenon = "on"
else
	xenon = "off"
end
if IsVehicleNeonLightEnabled(personalvehicle,0) then
	neon0 = "on"
else
	neon0 = "off"
end
if IsVehicleNeonLightEnabled(personalvehicle,1) then
	neon1 = "on"
else
	neon1 = "off"
end
if IsVehicleNeonLightEnabled(personalvehicle,2) then
	neon2 = "on"
else
	neon2 = "off"
end
if IsVehicleNeonLightEnabled(personalvehicle,3) then
	neon3 = "on"
else
	neon3 = "off"
end
if GetVehicleTyresCanBurst(personalvehicle) then
	bulletproof = "off"
else
	bulletproof = "on"
end
if GetVehicleModVariation(personalvehicle,23) then
	variation = "on"
else
	variation = "off"
end

TriggerServerEvent("CarShopAddTooDB", fakecar.model, plate, plateindex,primarycolor,secondarycolor,pearlescentcolor,wheelcolor,neoncolor1,neoncolor2,neoncolor3,windowtint,wheeltype,mods0,mods1,mods2,mods3,mods4,mods5,mods6,mods7,mods8,mods9,mods10,mods11,mods12,mods13,mods14,mods15,mods16,turbo,tiresmoke,xenon,mods23,mods24,neon0,neon1,neon2,neon3,bulletproof,smokecolor1,smokecolor2,smokecolor3,variation)
			--DealWithDB(personalvehicle)
			
		end
		vehshop[closeShop].opened = false
		vehshop[closeShop].menu.from = 1
		vehshop[closeShop].menu.to = 10
	end)
end

function DealWithDB(veh) 
						local colors = table.pack(GetVehicleColours(veh))
						local extra_colors = table.pack(GetVehicleExtraColours(veh))
						...
						TriggerServerEvent("carshop:UpdateVeh", plate, plateindex,primarycolor,secondarycolor,pearlescentcolor,wheelcolor,neoncolor1,neoncolor2,neoncolor3,windowtint,wheeltype,mods0,mods1,mods2,mods3,mods4,mods5,mods6,mods7,mods8,mods9,mods10,mods11,mods12,mods13,mods14,mods15,mods16,turbo,tiresmoke,xenon,mods23,mods24,neon0,neon1,neon2,neon3,bulletproof,smokecolor1,smokecolor2,smokecolor3,variation)
						cangoon = true
end

function drawMenuButton(button,x,y,selected)
	local menu = vehshop[closeShop].menu
...
function DoesPlayerHaveVehicle(model,button,y,selected)
		...
		if t then
			drawMenuRight("OWNED",vehshop[closeShop].menu.x,y,selected)
		else
			drawMenuRight(button.costs.."$",vehshop[closeShop].menu.x,y,selected)
		end
end
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustPressed(1,201) and IsPlayerInRangeOfVehshop() then
			if vehshop[closeShop].opened then
				CloseCreator()
			else
				OpenCreator()
			end
		...
		if vehshop[closeShop].opened then
			local ped = LocalPed()
			local menu = vehshop[closeShop].menu[vehshop[closeShop].currentmenu]
			Citizen.Trace(tostring(closeShop))
			Citizen.Trace(vehshop[closeShop].currentmenu)
			drawTxt(vehshop[closeShop].title,1,1,vehshop[closeShop].menu.x,vehshop[closeShop].menu.y,1.0, 255,255,255,255)
			drawMenuTitle(menu.title, vehshop[closeShop].menu.x,vehshop[closeShop].menu.y + 0.08)
			drawTxt(vehshop[closeShop].selectedbutton.."/"..tablelength(menu.buttons),0,0,vehshop[closeShop].menu.x + vehshop[closeShop].menu.width/2 - 0.0385,vehshop[closeShop].menu.y + 0.067,0.4, 255,255,255,255)
			local y = vehshop[closeShop].menu.y + 0.12
			buttoncount = tablelength(menu.buttons)
			local selected = false
			
			for i,button in pairs(menu.buttons) do
				if i >= vehshop[closeShop].menu.from and i <= vehshop[closeShop].menu.to then
					
					if i == vehshop[closeShop].selectedbutton then
						selected = true
					else
						selected = false
					end
					drawMenuButton(button,vehshop[closeShop].menu.x,y,selected)
					if button.costs ~= nil then
						if vehshop[closeShop].currentmenu == "compacts" or vehshop[closeShop].currentmenu == "coupes" or vehshop[closeShop].currentmenu == "sedans" or vehshop[closeShop].currentmenu == "sports" or vehshop[closeShop].currentmenu == "sportsclassics" or vehshop[closeShop].currentmenu == "super" or vehshop[closeShop].currentmenu == "muscle" or vehshop[closeShop].currentmenu == "offroad" or vehshop[closeShop].currentmenu == "suvs" or vehshop[closeShop].currentmenu == "vans" or vehshop[closeShop].currentmenu == "industrial" or vehshop[closeShop].currentmenu == "cycles" or vehshop[closeShop].currentmenu == "motorcycles" or vehshop[closeShop].currentmenu == "customCars" or vehshop[closeShop].currentmenu == "customBikes" or vehshop[closeShop].currentmenu == "workCars" then
		...
		if vehshop[closeShop].opened then
			...
			if IsControlJustPressed(1,188) then
				if vehshop[closeShop].selectedbutton > 1 then
					vehshop[closeShop].selectedbutton = vehshop[closeShop].selectedbutton -1
					if buttoncount > 10 and vehshop[closeShop].selectedbutton < vehshop[closeShop].menu.from then
						vehshop[closeShop].menu.from = vehshop[closeShop].menu.from -1
						vehshop[closeShop].menu.to = vehshop[closeShop].menu.to - 1
					end
				end
			end
			if IsControlJustPressed(1,187)then
				if vehshop[closeShop].selectedbutton < buttoncount then
					vehshop[closeShop].selectedbutton = vehshop[closeShop].selectedbutton +1
					if buttoncount > 10 and vehshop[closeShop].selectedbutton > vehshop[closeShop].menu.to then
						vehshop[closeShop].menu.to = vehshop[closeShop].menu.to + 1
						vehshop[closeShop].menu.from = vehshop[closeShop].menu.from + 1
					end
				end	
...
function ButtonSelected(button)
	...
	local this = vehshop[closeShop].currentmenu
	...
	if this == "main" then
		...
		elseif btn == "Detroit Imports" then
			OpenMenu('imports')
		end
	...
	elseif this == "imports" then
		if btn == "Custom Vehicles" then
			OpenMenu('customCars')
		elseif btn == "Custom Motorcycles" then
			OpenMenu('customBikes')
		end
	elseif this == "compacts" or this == "coupes" or this == "sedans" or this == "sports" or this == "sportsclassics" or this == "super" or this == "muscle" or this == "offroad" or this == "suvs" or this == "vans" or this == "industrial" or this == "cycles" or this == "motorcycles" or this == "customCars" or this == "customBikes" or this == "workCars" then
			TriggerServerEvent('CheckMoneyForCar', button.model, button.costs)
...
function OpenMenu(menu)
	...
	vehshop[closeShop].lastmenu = vehshop[closeShop].currentmenu
	if menu == "vehicles" then
		vehshop[closeShop].lastmenu = "main"
	elseif menu == "motorcycles"  then
		vehshop[closeShop].lastmenu = "main"
	elseif menu == "imports"  then
		vehshop[closeShop].lastmenu = "main"
	elseif menu == 'race_create_objects' then
		vehshop[closeShop].lastmenu = "main"
	elseif menu == "race_create_objects_spawn" then
		vehshop[closeShop].lastmenu = "race_create_objects"
	end
	vehshop[closeShop].menu.from = 1
	vehshop[closeShop].menu.to = 10
	vehshop[closeShop].selectedbutton = 0
	vehshop[closeShop].currentmenu = menu	
end
...
function Back()
	...
	backlock = true
	if vehshop[closeShop].currentmenu == "main" then
		CloseCreator()
	elseif vehshop[closeShop].currentmenu == "compacts" or vehshop[closeShop].currentmenu == "coupes" or vehshop[closeShop].currentmenu == "sedans" or vehshop[closeShop].currentmenu == "sports" or vehshop[closeShop].currentmenu == "sportsclassics" or vehshop[closeShop].currentmenu == "super" or vehshop[closeShop].currentmenu == "muscle" or vehshop[closeShop].currentmenu == "offroad" or vehshop[closeShop].currentmenu == "suvs" or vehshop[closeShop].currentmenu == "vans" or vehshop[closeShop].currentmenu == "industrial" or vehshop[closeShop].currentmenu == "cycles" or vehshop[closeShop].currentmenu == "motorcycles" or vehshop[closeShop].currentmenu == "customCars" or vehshop[closeShop].currentmenu == "customBikes" or vehshop[closeShop].currentmenu == "workCars" then
		if DoesEntityExist(fakecar.car) then
			Citizen.InvokeNative(0xEA386986E786A54F, Citizen.PointerValueIntInitialized(fakecar.car))
		end
		fakecar = {model = '', car = nil}
		if(vehshop[closeShop].currentmenu == "workCars") then
			CloseCreator()
		else
		OpenMenu(vehshop[closeShop].lastmenu)
		end
	else
		OpenMenu(vehshop[closeShop].lastmenu)
	end
...
AddEventHandler("FinishMoneyCheckForcar", function()
		boughtcar = true
		CloseCreator()
end)

RegisterNetEvent("vehShopGetVehReturn")
AddEventHandler("vehShopGetVehReturn", function(vehicles, vehs)
	vehshop = vehicles
	vehList = vehs
	loaded = true
end)