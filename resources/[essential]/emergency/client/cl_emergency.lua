				...
				if (Vdist(playerPos.x, playerPos.y, playerPos.z, x, y, z) < 2.0) then
					...
					if (IsControlJustReleased(1, 51)) then
						if (GetEntityModel(GetPlayerPed(-1)) == GetHashKey("mp_m_freemode_01")) then
							...
							exports.pNotify:SendNotification({text = "You need to be a mp_m_freemode_01 to be a paramedic", type = "success", queue = "left", timeout = 3000, layout = "centerRight"})
						end
					end
				end
			end
...
Citizen.CreateThread(
	function()
		...
		while true do
			...
			if (Vdist(playerPos.x, playerPos.y, playerPos.z, x, y, z) < 100.0) and isInService and jobId == 1 then
				-- Service car
				DrawMarker(1, x, y, z - 1, 0, 0, 0, 0, 0, 0, 3.0001, 3.0001, 1.5001, 255, 165, 0,165, 0, 0, 0,0)
...
AddEventHandler('es_em:sendEmergencyToDocs',
	function(reason, playerIDInComa, x, y, z, sourcePlayerInComa)
		...
		Citizen.CreateThread(
			function()
				if isInService and jobId == 1 and not playerInComaIsADoc then
					...
					Citizen.Trace("Display" .. reason)
					--SendNotification(txt[lang]['emergency'] .. reason)
					--SendNotification(txt[lang]['takeCall'])
					exports.pNotify:SetQueueMax("left", 20)
					exports.pNotify:SendNotification({text = "EMERGENCY Reason: " .. reason, type = "success", queue = "left", timeout = 15000, layout = "centerRight"})
					exports.pNotify:SendNotification({text = "Press Y to take call", type = "success", queue = "left", timeout = 15000, layout = "centerRight"})
					while not controlPressed do
						Citizen.Wait(0)
						notificationInProgress = true

						if (GetTimeDifference(GetGameTimer(), notifReceivedAt) > 15000) then
							exports.pNotify:SendNotification({text = "EMERGENCY  Call Expired", type = "success", queue = "left", timeout = 3000, layout = "centerRight"})
							notificationInProgress = false
							controlPressed = true
							Citizen.Trace("Call Expire")
...
AddEventHandler('es_em:callTaken',
	function(playerName, playerID, x, y, z, sourcePlayerInComa)
		...
		if isInService and jobId == 1 and not playerInComaIsADoc then
			Citizen.Trace("Call Taken")
			Citizen.Trace(tostring(playerID))
			Citizen.Trace(tostring(playerServerId))
			Citizen.Trace(tostring(sourcePlayerInComa))
			--SendNotification(txt[lang]['callTaken'] .. playerName .. '~s~')
			StartEmergency(x, y, z, sourcePlayerInComa)
			exports.pNotify:SendNotification({text = txt[lang]['callTaken'] .. playerName, type = "success", queue = "left", timeout = 3000, layout = "centerRight"})
		end

		if playerServerId == playerID then
			Citizen.Trace("Playtobe")
...
function StartEmergency(x, y, z, sourcePlayerInComa)
	...
	SetBlipSprite(BLIP_EMERGENCY, 61)
	--SetNewWaypoint(x, y)
	SetBlipRoute(BLIP_EMERGENCY, true)

	--SendNotification(txt[lang]['gps'])
	exports.pNotify:SendNotification({text = txt[lang]['gps'], type = "success", queue = "left", timeout = 3000, layout = "centerRight"})
	Citizen.CreateThread(
		function()
			local isRes = false
			...
			while not isRes do
				Citizen.Wait(0)
				if (GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), x,y,z, true)<100.0) then
					DrawMarker(1, x, y, z - 1, 0, 0, 0, 0, 0, 0, 3.0001, 3.0001, 1.5001, 4, 29, 155,165, 0, 0, 0,0)
				if (GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), x,y,z, true)<3.0) then
					...
					if (IsControlJustReleased(1, Keys['E'])) then
						...
						isRes = true
						SetBlipRoute(BLIP_EMERGENCY, false)
...
function GetService()
	...
	if isInService then
		...
		TriggerServerEvent("jobssystem:jobs", 10)
		if(BLIP_EMERGENCY) then
			RemoveBlip(BLIP_EMERGENCY)
		end
	else
