...
local txt = {
  ['fr'] = {
	...
	['en'] = {
		...
		['ambIsComming'] = 'An Ambulance is on the Way!',
...
AddEventHandler('baseevents:onPlayerDied', function(playerId, reasonID)
	Citizen.Trace("Boldead")
	local weaToCheck = {"WEAPON_KNIFE", "WEAPON_NIGHTSTICK", "WEAPON_HAMMER", "WEAPON_BAT", "WEAPON_GOLFCLUB", "WEAPON_CROWBAR", "WEAPON_PISTOL", "WEAPON_COMBATPISTOL", "WEAPON_APPISTOL", "WEAPON_PISTOL50",
	...
	"WEAPON_ASSAULTSHOTGUN", "WEAPON_BULLPUPSHOTGUN", "WEAPON_STUNGUN", "WEAPON_SNIPERRIFLE", "WEAPON_HEAVYSNIPER", "WEAPON_GRENADELAUNCHER", "WEAPON_RPG", "WEAPON_STINGER", "WEAPON_MINIGUN",
	"WEAPON_GRENADE", "WEAPON_STICKYBOMB","WEAPON_PIPEBOMB", "WEAPON_SMOKEGRENADE", "WEAPON_BZGAS", "WEAPON_MOLOTOV", "WEAPON_FIREEXTINGUISHER", "WEAPON_PETROLCAN", "WEAPON_BALL", "WEAPON_FLARE"
	}
	weaToGive  = {}
	local reason = txt[lang]['accident']
	for i, v in ipairs(weaToCheck) do		
		if(HasPedGotWeapon(GetPlayerPed(-1),GetHashKey(v),false)) then 
			table.insert(weaToGive, (v))
			print(v)
		end
	end
	OnPlayerDied(playerId, reasonID, reason, pedWapons)
end)
...
local wasCop = false
RegisterNetEvent('wasCopForEm')
AddEventHandler('wasCopForEm',
	function()
		wasCop = true
	end
)

RegisterNetEvent('giveOtherWeapons')
AddEventHandler('giveOtherWeapons', function()			
	for i, v in ipairs(weaToGive) do
...
function ResPlayer(weaToCheck1)
	isRes = true
	...
	ClearPedTasksImmediately(playerPed)
	SetEntityCoords(GetPlayerPed(-1), 1150.65405273, -1525.449707, 33.843399047852)
	--NetworkResurrectLocalPlayer(1150.65405273, -1525.449707, 33.843399047852, true, true, false)
	if(wasCop) then
		wasCop = false
		TriggerServerEvent("mm:spawn")
	end
...
function OnPlayerDied(playerId, reasonID, reason, pedWapons1)
	...
	Citizen.CreateThread(
		function()
			...
			local timeAtDeath = GetGameTimer()
			...
			while not isRes do
				...
				if (IsControlJustReleased(1, Keys['X'])) then
					Citizen.Trace("X Pressed")
					if(isDocConnected and (GetTimeDifference(GetGameTimer(), timeAtDeath) > 300000)) then
						ResPlayer(pedWapons1)
					elseif (not isDocConnected) then
						ResPlayer(pedWapons1)
					elseif(isDocConnected and (GetTimeDifference(GetGameTimer(), timeAtDeath) < 300000)) then
						--Citizen.Trace("gottoMustWait")
						local timediff = GetTimeDifference(GetGameTimer(), timeAtDeath)
						local milliLeft = 300000 - timediff
						local toSec = math.floor((milliLeft / 1000) + 0.5)
						--Citizen.Trace("You must wait "..tostring(toSec).." seconds to respawn when Medics are on")						
						SendNotification("You must wait " .. tostring(toSec) .. " seconds to respawn when Medics are on")						
					end
				end
