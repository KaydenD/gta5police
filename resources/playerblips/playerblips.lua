...
function DrawText3D(x,y,z, text, talking) -- some useful function, use it if you want!
                ...
                if ((distance < disPlayerNames)) then
                    if not (ignorePlayerNameDistance) then
						...
						if (NetworkIsPlayerTalking(id)) then
                        	DrawText3D(x2, y2, z2+1, tostring(GetPlayerServerId(id)), true)
							--Citizen.Trace("draw")
						else
							DrawText3D(x2, y2, z2+1, tostring(GetPlayerServerId(id)), false)
							--Citizen.Trace("draw not talk")
						end
                        -- For just the player's source id use this or use the line above for name and source id | DrawText3D(x2, y2, z2+1, GetPlayerServerId(id))
